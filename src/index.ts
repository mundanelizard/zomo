...
app.get("/refresh", async function (req, res) {
  UpdateService.getInstance()
    ...
    .then((result) => sendSuccess(res, result, "Successfully refreshed cache."))
...
app.get("/download", async function (req, res) {
  UpdateService.getInstance()
    ...
    .then((result) =>
      sendSuccess(res, null, { Location: result.location }, 302)
    )
    .catch((err) => sendError(res, err));
});
...
app.get(
  ...
  async function (_, res) {
    UpdateService.getInstance()
      .releases()
      .then((result) => sendSuccess(res, result.content, result.meta))
      .catch((err) => sendError(res, err));
  }
);
